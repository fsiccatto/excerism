...
export const clean = (number) => {
  number = number.replace(/[+()-\s.]/g, ''); // Eliminamos todos los caracteres
  number = number.replace(/1(?=\d{10})/g, ''); // Reemplazamos el 1 del principio, solo si esta seguido de 10 numeros

  errors.forEach((message, reg) => {
    if (reg.test(number)) throw new Error(message);
  })
  return number;
};
